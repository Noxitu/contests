...
struct Field
{
    ...
    constexpr long long operator()(int t) { return 1LL*a*t + b; }
    friend istream& operator>>(istream &in, Field &r) { return in >> r.a >> r.b; }
};

typedef pair<long long, list<Field>::iterator> Pair;

void test()
{
    int n;
    cin >> n;
    vector<Field> fieldsv(n);
    vector<long long> days(n, 0);
    cin >> fieldsv;
    
    sort(begin(fieldsv), end(fieldsv), compare_field(&Field::a));

    list<Field> fields(begin(fieldsv), end(fieldsv));

    long long answer = 0;
    {
        int day = 0;
        for(auto &f : fields) answer += f(day++);
    }
    ...
    for(int k = n-1; k >= 0; --k)
    {
        ...
        Pair removal_cost = {fields.front()(0), begin(fields)};
        {
            int day = 1;
            for(auto it = next(begin(fields)); it != end(fields); ++it)
            {
                removal_cost.first += it->a;
                if(removal_cost.first > (*it)(day))
                    removal_cost = Pair((*it)(day), it);
                ++day;
            }
        }

        fields.erase(removal_cost.second);
        answer -= removal_cost.first;
    }

    copy( begin(days), end(days), ostream_iterator<long long>(cout, "\n") );
}